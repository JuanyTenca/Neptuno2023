...
namespace Neptuno2023.Datos.Repositorios
{
    public class RepositorioProveedores : IRepositorioProveedores
    {
        ...
        public List<Proveedor> GetProveedores(string textoFiltro)
        {
            try
            {
                List<Proveedor> lista = new List<Proveedor>();
                ...
                {
                    conn.Open();
                    string selectQuery = "SELECT ProveedorId, NombreProveedor, Direccion, CodPostal, PaisId, CiudadId, Telefono, Email FROM Proveedores WHERE NombreProveedor LIKE @textoFiltro ORDER BY NombreProveedor";
                    using (var comando = new SqlCommand(selectQuery, conn))
                    {
                        ...
                        using (var reader = comando.ExecuteReader())
                        {
                            ...
                            while (reader.Read())
                            {
                                var pais = ConstruirProveedor(reader);
                                lista.Add(pais);
                            }
                        }
                    }
